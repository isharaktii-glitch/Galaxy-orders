...
export default function AdminUsersPage() {
  ...
  const verifyKyc = async (id: string) => {
    await fetch('/api/admin/users', { method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({id,kycVerified:true}) });
            ...
            <tbody className="divide-y divide-gray-800">
              {users.map((user) => (
                <motion.tr key={user.id} initial={{opacity:0}} animate={{opacity:1}} className="hover:bg-gray-900/50 transition-colors">
                  ...
                  <td className="px-4 py-4"><p className="text-gray-400 text-xs">{user.phone}</p></td>
                  <td className="px-4 py-4">
                  ...
                  <td className="px-4 py-4">
                    <div className="flex gap-2">
                      ...
                      {!user.kycVerified && (
                        <button onClick={()=>verifyKyc(user.id)} className="p-2 rounded-lg bg-blue-500/20 text-blue-400 hover:bg-blue-500/30 transition-colors">
