...
import { LayoutDashboard, Users, Package, ShoppingCart, CreditCard, Tag, Megaphone, Settings, LogOut, Gavel, Menu, X, Zap, Building2 } from 'lucide-react';
...
export default function AdminSidebar() {
  ...
  return (
    <aside className={`${open?'w-64':'w-20'} transition-all duration-300 glass border-r border-indigo-500/20 flex flex-col h-full`}>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all ${isActive?'bg-indigo-600 text-white':'text-gray-400 hover:text-white hover:bg-gray-800'}`}>
