...
export default function AdminCategoriesPage() {
  ...
  const addCategory = async () => {
    if (!newCat.name) { toast.error('Enter name'); return; }
    await fetch('/api/categories', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(newCat) });
    toast.success('Category added!');
    setNewCat({name:'',nameSi:'',nameTa:'',icon:''});
    fetchCats();
  };

  const toggleVisibility = async (id: string, isVisible: boolean) => {
    await fetch('/api/categories', { method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({id,isVisible:!isVisible}) });
    fetchCats();
  };

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-black gradient-text">Categories</h1>
      <div className="glass rounded-2xl p-6 neon-border">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[{key:'name',ph:'Name (English) *'},{key:'nameSi',ph:'Name (Sinhala)'},{key:'nameTa',ph:'Name (Tamil)'},{key:'icon',ph:'Icon (emoji)'}].map(f=>(
            <input key={f.key} placeholder={f.ph} value={(newCat as any)[f.key]} onChange={(e)=>setNewCat({...newCat,[f.key]:e.target.value})}
        ...
        <button onClick={addCategory} className="btn-primary text-white mt-4">Add Category</button>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {categories.map((cat) => (
          <div key={cat.id} className={`glass rounded-2xl p-5 card-3d border-2 ${cat.isVisible?'border-indigo-500/30':'border-gray-700 opacity-60'}`}>
            <div className="flex items-start justify-between mb-3">
              ...
              <button onClick={()=>toggleVisibility(cat.id,cat.isVisible)}
                className={`p-2 rounded-xl transition-colors ${cat.isVisible?'bg-indigo-500/20 text-indigo-400':'bg-gray-700 text-gray-400'}`}>
