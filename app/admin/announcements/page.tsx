...
export default function AdminAnnouncementsPage() {
  ...
  const send = async () => {
    ...
    setLoading(true);
    const res = await fetch('/api/announcements', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({title,message,targetRole:targetRole||undefined}) });
    if (res.ok) { toast.success('Sent!'); setTitle(''); setMessage(''); setTargetRole(''); }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <h1 className="text-2xl font-black gradient-text">Announcements</h1>
      <div className="glass rounded-2xl p-6 neon-border">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-gray-400 text-xs">Send to all or specific groups</p>
          </div>
        </div>
        <div className="space-y-4">
          ...
          <button onClick={send} disabled={loading} className="btn-primary text-white w-full flex items-center justify-center gap-2">
            <Send className="w-5 h-5"/> {loading?'Sending...':'Send Announcement'}
          </button>
        </div>
      </div>
    </div>
